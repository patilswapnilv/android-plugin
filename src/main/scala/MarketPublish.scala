...
trait MarketPublish extends AndroidProjectOld {
  ...
  def zipAlignAction = execTask {
    <x>
      { zipAlignPath }
      -v  4{ packageApkPath }{ packageAlignedPath }
    </x>
  } dependsOn (signRelease, cleanAligned) describedAs ("Run zipalign on signed jar.")

  lazy val signRelease = signReleaseAction
  def signReleaseAction = execTask {
    <x>
      jarsigner -verbose -keystore{ keystorePath }
      -storepass{ getPassword }{ packageApkPath }{ keyalias }
    </x>
  } dependsOn (packageRelease) describedAs ("Sign with key alias '%s' in %s, using jarsigner." format (keyalias, keystorePath))
